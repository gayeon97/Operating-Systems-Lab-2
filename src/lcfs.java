...
//		printSummaryData(overallFinishTime, cpuUsedTime, ioUsedTime, throughput, totalTurnaroundT, totalWaitingT, numProcesses);
		...
		//now done with RR, no longer preemptive
		quantum = 0;
		...
		while (numPTerminated < numProcesses) {
			...
			//addToReadyQ(readyAtSameCycleFCFSQ, readyFCFSQ); //add to the readyQ
		...
		for (int i = 0; i < t2.size(); i ++) {
			trial.add(indexT, t2.poll());
			i --;
			indexT ++;
		}
		
		System.out.println(Arrays.toString(trial.toArray()));
		
		System.out.println(trial.remove(0));
